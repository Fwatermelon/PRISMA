    ...
    def _encode_sample(self, idxes):
        ...
        try:
            return (
                ...
                np.array(actions),
                np.array(rewards),
                np.array(obses_tp1, dtype=object),
                np.array(dones),
                np.ones(len(idxes), dtype=np.float32),
            )
        except:
            print("ERROR")
            print(obses_t)
...
class ReplayBuffer_(object):
    ...
    def _encode_sample(
        self,
        idxes: Union[List[int], np.ndarray],
    ):
        obses_t, actions, rewards, obses_tp1, dones = [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
            ...
            dones.append(done)
        return (
            self._normalize_obs(np.array(obses_t)),
            np.array(actions),
...
class PrioritizedReplayBuffer(ReplayBuffer_):
    ...
    def update_priorities(self, idxes, priorities):
        ...
        values = (
            np.array(self.original_prios)[idxes] / self.latest_gradient_step[priorities]
        ) ** self._alpha
