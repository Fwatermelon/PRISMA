...
NS_LOG_COMPONENT_DEFINE ("SmallSignalingPacketManager");

SmallSignalingPacketManager::SmallSignalingPacketManager ()
{
  PacketManager();
  NS_LOG_FUNCTION (this);
}

SmallSignalingPacketManager::SmallSignalingPacketManager (Ptr<Node> node, vector<int> neighbors) : PacketManager(node, neighbors)
{
  NS_LOG_FUNCTION (this);
}
...
SmallSignalingPacketManager::receivePacket(Ptr<Packet> packet){
  ...
  m_signalingPacketSource = tagCopy.GetSource();
  m_signalingPacketisFirst = tagCopy.GetIsFirst();
  // m_oneHopDelay = tagCopy.GetOneHopDelay();
  return m_arrivedAtFinalDest && ret;
}

// float
// SmallSignalingPacketManager::getReward()
// {
...
SmallSignalingPacketManager::getInfo()
{
  ...
  myInfo += ", Arrived from="; //20
  myInfo += std::to_string(m_signalingPacketSource);
  myInfo += ", Is Source="; //21
  myInfo += std::to_string(m_signalingPacketisFirst);
  return myInfo;
}
