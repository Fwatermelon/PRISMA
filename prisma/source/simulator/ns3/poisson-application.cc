...
void PoissonGeneratorApplication::UpdateAvgTrafficRate(){
  ...
  // increase the traffic between src 2 and dest 4
  if (m_src == 6 && m_dest == 2){
    m_avgRate = DataRate (m_avgRate.GetBitRate() *150.0);
  }
  if (m_src == 2 && m_dest == 6){
    m_avgRate = DataRate (m_avgRate.GetBitRate() *150.0);
  }
  // NS_LOG_UNCOND("traffic between "<<m_src<<" and "<<m_dest<<"    "<<m_avgRate.GetBitRate());
  // if (m_src == 18 && m_dest == 23){
  //   NS_LOG_UNCOND("UPDATING TRAFFIC RATE at " << Simulator::Now().GetSeconds() << "s" << "   "<<m_avgRate.GetBitRate());
  //   m_avgRate = DataRate (500000.0);
  // }
  // if (m_src == 15 && m_dest == 5){
  //   NS_LOG_UNCOND("UPDATING TRAFFIC RATE at " << Simulator::Now().GetSeconds() << "s" << "   "<<m_avgRate.GetBitRate());
  //   m_avgRate = DataRate (500000.0);
  // }

  // if (m_src == 18 && m_dest == 23){
  //   m_avgRate = DataRate (m_avgRate.GetBitRate() *1000.0);
  // }
...
void PoissonGeneratorApplication::SendPacket ()
{
  ...
  // NS_LOG_UNCOND("PoissonGeneratorApplication::SendPacket: "<<m_src<<"->"<<m_dest<<"    "<<m_avgRate.GetBitRate() <<"    "<<m_pktSize<<"    "<<uint64_t(Simulator::Now().GetSeconds()) << "    "<< (uint32_t) tag2.GetTrafficValable() << "    "<< (uint32_t) tag2.GetRejectedPacket() << "    "<< (uint32_t) tag2.GetFinalDestination() << "    "<< (uint32_t) tag2.GetSource() << "    "<< (uint32_t) tag2.GetNextHop() << "    "<< (uint32_t) tag2.GetLastHop() << "    "<< (double) tag2.GetStartTime() <<" "<< (uint32_t) tag2.GetSimpleValue());
