...
PacketRoutingEnv::PacketRoutingEnv (Ptr<Node> node, NodeContainer nodes, uint64_t linkRateValue, bool activateSignaling, double signPacketSize, vector<int> overlayNeighbors, int controller_id)
{
  NS_LOG_FUNCTION (this);
  ...
  m_dataPacketManager = new DataPacketManager(node, overlayNeighbors, nodes, controller_id);
...
PacketRoutingEnv::GetObservation()
{
  ...
  if(is_trainStep_flag==0) {
    ...
    if (m_packetType==DATA_PACKET) {
      return m_dataPacketManager->getObservation();
    }
...
{
  // NS_LOG_UNCOND("PacketRoutingEnv::GetExtraInfo: " << ", time="<<Simulator::Now().GetSeconds() << ", m_dataPacketManager->m_node->GetId()="<<m_dataPacketManager->m_node->GetId() << " " << int(m_packetType));
  std::string myInfo;
  // NS_LOG_UNCOND("PacketRoutingEnv::GetExtraInfo: " << ", time="<<Simulator::Now().GetSeconds() << ", m_dataPacketManager->m_node->GetId()="<<m_dataPacketManager->m_node->GetId() << " " << int(m_packetType) << " " << is_trainStep_flag);
  ...
  if(m_packetType==TARGET_UPDATE_PACKET){
  ...
  if(m_packetType==REPLAY_MEMORY_UPDATE_PACKET){
...
PacketRoutingEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  if(m_packetType==DATA_PACKET){
    if (is_trainStep_flag==1){
      ...
      if(m_train){
        //send small signaling if packet not new
        // NS_LOG_UNCOND("PacketRoutingEnv::ExecuteActions: action_int="<<action_int << ", time="<<Simulator::Now().GetSeconds());
        // NS_LOG_UNCOND("PacketRoutingEnv::ExecuteActions: action_int="<<action_int << ", time="<<Simulator::Now().GetSeconds() << ", m_dataPacketManager->m_controller_address="<<m_dataPacketManager->m_controller_address << ", m_lastHop="<<m_lastHop << " , m_dataPacketManager->m_node->GetId()="<<m_dataPacketManager->m_node->GetId());
        if (action_int< 1000){
          if (m_lastHop < 1000){ // send back the control packet
            m_dataPacketManager->sendSmallSignalingPacket(0);
        ...
        //  if signalingType is centralized, send a small signaling packet to the controller
        if (m_dataPacketManager->m_controller_address!= -1){
          m_dataPacketManager->sendSmallSignalingPacket(1);
        }
     }
     //Send data packet
...
{  
  ...
  // NS_LOG_UNCOND("Before Receiving "<< entity->m_packetType << " packet, time="<<Simulator::Now().GetSeconds() << ", m_dataPacketManager->m_node->GetId()="<<entity->m_dataPacketManager->m_node->GetId() << ", packet size="<<packet->GetSize()<<", packet type="<< int(entity->m_packetType)<<", packet source " << tagCopy.GetSource()<<", packet destination " << tagCopy.GetFinalDestination()<<", packet uid " << packet->GetUid()<<", packet last hop " << tagCopy.GetLastHop()<<", packet next hop " << tagCopy.GetNextHop()<<", packet start time " << tagCopy.GetStartTime() << " trafficvalable " << int(tagCopy.GetTrafficValable()));
  bool valid = true;
  if(entity->m_packetType==DATA_PACKET){
    valid = entity->m_dataPacketManager->receivePacket(p, netDev);
  } else if(entity->m_packetType==TARGET_UPDATE_PACKET){
    valid = entity->m_bigSignalingPacketManager->receivePacket(p);
  } else if(entity->m_packetType==REPLAY_MEMORY_UPDATE_PACKET){
