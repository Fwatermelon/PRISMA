...
bool PacketManager::receivePacket(Ptr<Packet> packet){
    // NS_LOG_UNCOND("PacketManager::receivePacket - "<< m_node->GetId()<< " " << packet->ToString());
    //Packet copy
    Ptr<Packet> p;
    p = packet->Copy();
    ...
    // NS_LOG_UNCOND("We received a packet of type "<< m_packetType << " from "<< m_lastHop << " to "<< m_nextHop << " at "<< m_node->GetId() << " with packet size "<< m_packetSize << " and packet uid "<< m_packetUid << " at time "<< Simulator::Now().GetSeconds()<< " with source "<< m_source << " and destination "<< m_destination << " and packet type "<< int(m_packetType));
