...
int main (int argc, char *argv[])
{
  ...
  // TODO: put the string to dynamic size because now if the path is too long it will crash
  std::string adj_mat_file_name ("/app/prisma/examples/overlay_full_mesh_10n_geant/topology_files/physical_adjacency_matrix.txt");
  std::string overlay_mat_file_name ("/app/prisma/examples/overlay_full_mesh_10n_geant/topology_files/overlay_adjacency_matrix.txt");
  std::string node_coordinates_file_name ("/app/prisma/examples/overlay_full_mesh_10n_geant/topology_files/node_coordinates.txt");
  std::string node_intensity_file_name("/app/prisma/examples/overlay_full_mesh_10n_geant/traffic_matrices/node_intensity_normalized_0.txt");
  std::string map_overlay_file_name("/app/prisma/examples/overlay_full_mesh_10n_geant/topology_files/overlay_map.txt");
  std::string tunnels_max_delays_file_name("/app/prisma/examples/overlay_full_mesh_10n_geant/topology_files/max_observed_values.txt");
  std::string opt_rejected_file_name("/app/prisma/examples/overlay_full_mesh_10n_geant/optimal_solution/rejected_flows.txt");
  ...
  std::string monitoring_type = "active";
  ...
  cmd.AddValue ("NNSize", "total size of the weights of the NN in bytes", NNSize);
  ...
  cmd.AddValue ("controller_id", "Controller ID when using centralized signaling type", controller_id);
  cmd.AddValue ("monitoring_type", "Monitoring type", monitoring_type);
  ...
  // if it is centralized signaling and train is true, then we need to ensure that the controller is connected to all the nodes via a a direct virtual link
  if (signalingType=="centralized" && train){
    for (int i=0; i<overlay_n_nodes; i++){
      if (i!=controller_id){
        OverlayAdj_Matrix[controller_id][i] = 1;
        OverlayAdj_Matrix[i][controller_id] = 1;
      }
    }
  }
  ...
  if(agentType=="shortest_path" || agentType=="oracle_routing" || signalingType=="ideal"){
  ...
  if(signalingType=="NN"){
    NS_LOG_UNCOND("Setting up Replay Memory Update Packets...");
    for(int i=0;i<overlay_n_nodes;i++){
      smallSignalingSize[i] = (8 * (overlay_nodes_degree[i]+1)) + 17; // 8 bytes for the reward and (8 * (overlay_nodes_degree[i]+1)) for the state + 1 byte flag + 8 bytes for the packet id
    }
  } else if(signalingType=="target"){
    for(int i=0;i<overlay_n_nodes;i++){
      smallSignalingSize[i] = 25; // 8 bytes for the reward + 1 byte flag + 8 bytes for the packet id + 8 bytes for the target
    }
  }
    else if(signalingType=="logit"){
      for(int i=0;i<overlay_n_nodes;i++){
        if (DT_SendAllDestinations){
          smallSignalingSize[i] = 8 + (8 * (2* overlay_nodes_degree[i] + 1) * (overlay_n_nodes - 1));
        } else {
          NS_LOG_UNCOND("Setting up Replay Memory Update Packets..." << overlay_nodes_degree[i] << " " << overlay_n_nodes << " " << i);
          smallSignalingSize[i] = (8 * (2* overlay_nodes_degree[i] + 1)) + 17; // 8 bytes for the reward and 8 * (2* overlay_nodes_degree[i] + 1) for the state and logits + 8 bytes for the packet id + 1 byte for the flag
        }
    }
  }
    else if(signalingType=="centralized"){
        for(int i=0;i<overlay_n_nodes;i++){
          smallSignalingSize[i] = (8 * (overlay_nodes_degree[i]+1)) + 17; // 8 bytes for the reward and (8 * (overlay_nodes_degree[i]+1)) for the state + 1 byte flag + 8 bytes for the packet id
  ...
  NS_LOG_UNCOND("Creating link between overlay nodes in overlay network");
  ...
  for(int i=0;i<n_nodes;i++){
    NS_LOG_UNCOND(" " <<i<<" "<<underlay_to_overlay_map[i] << " " << n_nodes << " " << overlay_n_nodes);
    if(underlay_to_overlay_map[i]>=0){
      overlayNodesChecker[i] = 1;
      overlay_to_underlay_map[underlay_to_overlay_map[i]]=i;
    }
  }
  NS_LOG_UNCOND("Creating overlay network map");
  ...
  for (int i = 0; i < overlay_n_nodes; i++)
  {
    ...
    int underlay_controller_id = -1;
    if (signalingType=="centralized"){
      underlay_controller_id = overlay_to_underlay_map[controller_id];
    }

    packetRoutingEnv = CreateObject<PacketRoutingEnv> (n, nodes_switch, linkRateValue, activateSignaling, smallSignalingSize[i], overlayNeighbors[i], underlay_controller_id); // event-driven step
    ...
    // activate ping when the monitoring type is active
    if (monitoring_type=="active"){
    ...
    if (pingAsObs){
      NS_LOG_UNCOND("Setting max delays..");
  ...
  NS_LOG_UNCOND("Create n*(n-1) CBR Flows.");
          ...
          if (i != j && ActivateTrafficRate[i][j]>0 && DataRate(Traff_Matrix[i][j]).GetBitRate()>0)
            {
              ...
              if(true){
                ...
                if (opt)
                {
                  poisson.SetRejectedProbability(OptRejected[i][j]);
                }
                else
                {
                  poisson.SetRejectedProbability(0.0);
                }
                poisson.SetUpdatable(perturbations, 150.0);
              ...
              // TODO: enable target update control packets
              if(train && activateSignaling && overlayNodesChecker[i] && overlayNodesChecker[j]){
                if(signalingType == "NN"){
                  ...
                  if(OverlayAdj_Matrix[underlay_to_overlay_map[i]][underlay_to_overlay_map[j]]==1 ){
                    ...
                    sign.SetAverageStep (syncStep, NNSize); 
                ...
                // if centralized, send the target update control packet only from the controller node to the other nodes
                else if (signalingType=="centralized"){
                  if (overlay_to_underlay_map[controller_id] == i){
                      string string_ip_bigSignaling= "10.2.2."+std::to_string(j+1);
                      Ipv4Address ip_big_signaling(string_ip_bigSignaling.c_str());
                      sinkAddress = InetSocketAddress (ip_big_signaling, sinkPortUDP);
                      ...
                      BigSignalingAppHelper sign ("ns3::UdpSocketFactory",sinkAddress);
                      sign.SetAverageStep (syncStep, NNSize); 
                      sign.SetSourceDest(overlay_to_underlay_map[controller_id]+1, j+1, controller_id+1); 
                      ApplicationContainer apps = sign.Install (nodes_traffic.Get (overlay_to_underlay_map[controller_id]));  // traffic sources are installed on controller node
                      apps.Start (Seconds (AppStartTime )); 
                      apps.Stop (Seconds (AppStopTime));
                  }
...
{
  ifstream adj_mat_file;
  NS_LOG_UNCOND("Reading OptRejected file: "<<opt_rejected_file_name);
