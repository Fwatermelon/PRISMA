...
for session_name in os.listdir(f"{root_path}/{exps_folder_name}/"):
    ...
    G_overlay = nx.from_numpy_array(
        np.loadtxt(overlay_topology_path), create_using=nx.DiGraph()
    )

    ## load underlay topology
    underlay_topology_path = f"{root_path}/adjacency_matrix.txt"
    G_underlay = nx.from_numpy_array(
        np.loadtxt(underlay_topology_path), create_using=nx.DiGraph()
    )
    ...
    for idx_edge, edge in enumerate(G_overlay.edges):
        ...
        underlay_path = nx.shortest_path(
            G_underlay, underlay_node_src, underlay_node_dst
        )

        ## sum the values of the
        for idx in range(len(underlay_path) - 1):
            index_in_raw_data = list(G_underlay.edges).index(
                (underlay_path[idx], underlay_path[idx + 1])
            )
            delays_data[:, idx_edge + 1] += (
                (raw_data[:, index_in_raw_data + 1] + pkt_size) * 8 / link_cap
            ) + hop_delay
        np.savetxt(
            f"{root_path}/{exps_folder_name}/{session_name}/ground_truth_delay_{virtual_node_src}_{interface_idx}.txt",
            delays_data[:, [0, idx_edge + 1]],
        )
