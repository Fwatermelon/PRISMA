...
if __name__ == "__main__":
    ...
    sp_values = np.array(
        sp_df[sp_df.name == eval_metric_name].sort_values("step").value.values,
        dtype=float,
    )
    ...
    opt_values = np.array(
        opt_df[opt_df.name == eval_metric_name].sort_values("step").value.values,
        dtype=float,
    )
    ...
    for exp_name in os.listdir(dir_path):
        ...
        exp_values = np.array(
            exp_df[exp_df.name == eval_metric_name].sort_values("step").value.values,
            dtype=float,
        )
        ...
        exp_dict = dict(flag=exp_flag, mse=exp_mse, exp_values=exp_values)
...
sys.path.append("../")
from prisma.source.auxilliaries.utils import convert_tb_data
...
for pingPacketIntervalTime in pingPacketIntervalTimes:
    ...
    for model in list(models.keys()) + ["sp", "opt"]:
        ...
        for sync_step in sync_steps:
            ...
            data = df[df.name == "test_global_cost"].value.values[
                np.argsort(df[df.name == "test_global_cost"].step.values)
            ]
        ...
        # plt.legend()
        results_dict[f"{model}_{pingPacketIntervalTime}"] = np.mean(
            np.array(over_steps), axis=0
        )
        plt.plot(
            np.sort(df[df.name == "test_global_loss_rate"].step.values),
            np.min(np.array(over_steps), axis=0),
            label=f"{model}",
        )
