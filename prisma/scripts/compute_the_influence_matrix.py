...
overlay_topology_path = (
    "/home/tiago/Documents/PRISMA/prisma/examples/abilene/adjacency_matrix_2_5n.txt"
)
...
underlay_topology_path = (
    "/home/tiago/Documents/PRISMA/prisma/examples/abilene/adjacency_matrix.txt"
)
...
for idx_edge, edge in enumerate(G_overlay.edges):
    ...
    for idx in range(len(underlay_path) - 1):
        nodes_used_links[virtual_node_src].add(
...
for ni in G_overlay.nodes:
    for nj in G_overlay.nodes:
        if ni == nj:
            continue
        influence_matrix[ni][nj] = len(
            nodes_used_links[ni].intersection(nodes_used_links[nj])
        )
