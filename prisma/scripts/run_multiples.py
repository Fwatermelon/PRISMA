                            ...
                            for ping_freq in ping_freqs:
                                for mv_avg_interval in mv_avg_intervals:
                                    for trainin_load in training_loads:
                                        ...
                                        os.system(
                                            f'tsp ./scripts/run_5_nodes.sh {sync_step} {seed} {traff_mat} dqn_buffer{state} NN {ping_freq} {rb_size} {inc} abilene 0 {trainin_load} 1 350 "{session_name}" {experiment_name} {mv_avg_interval} {lr} {bs} {exploration[1]} {exploration[2]}'
                                        )
                                        inc += 1
print(inc)
