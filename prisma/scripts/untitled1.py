...
if __name__ == "__main__":
    ...
    for stat_idx in range(len(stat_names)):
        ...
        for i in range(len(names)):
            for charge_index, charge in enumerate(list_charges):
                ...
                for k in [0]:
                    if k == 0:
                        delay_temp = np.array(
                            pd.read_csv(
                                f"D:\\NetSim_\\NetSim\\prisma\\examples\\abilene\\tests_\\{names[i]}_load_{charge}.txt"
                            ).values[:, 5 + stat_idx],
                            dtype=float,
                        ).reshape(1, -1)
                    else:
                        delay_temp = np.concatenate(
                            (
                                delay_temp,
                                np.array(
                                    pd.read_csv(
            ...
            bpl = plt.boxplot(
                x=np.transpose(delay[:, :20]),
                sym="",
                widths=0.5,
                positions=np.array(range(len(list_charges))) * len(names)
                + increment[j],
            )
            # if i not in (0, 6):
            #     seed = i -1
            ...
            j += 1
        plt.xticks(
            range(0, len(list_charges) * (len(names)), len(names)),
            np.array((np.array(list_charges)), dtype=int),
        )
        plt.legend(loc=2)
