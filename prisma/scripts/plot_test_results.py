...
dir_path = (
    "/home/redha/PRISMA_copy/prisma/examples/abilene/ITC_NN_size_variations_experiment"
)
    ...
    for idx, dqn_model in enumerate(dqn_models):
        ...
        if signaling_type in ["sp", "opt"]:
            ...
            ax.plot(
                sync_steps,
                np.array(accu),
                label=f"{signaling_type} {dqn_model}",
                marker="o",
            )
        ...
        for traff_mat in traff_mats:
            for sync_step in sync_steps:
                ...
                try:
                    ...
                    exp_df = convert_tb_data(path)
                    injected = (
                        exp_df[exp_df.name == "overlay_data_pkts_injected_bytes_time"]
                        .sort_values("step")
                        .iloc[-1]
                        .value
                    )
                    big_s = (
                        exp_df[exp_df.name == "overlay_big_signalling_bytes"]
                        .sort_values("step")
                        .iloc[-1]
                        .value
                    )
                    small_s = (
                        exp_df[exp_df.name == "overlay_small_signalling_bytes"]
                        .sort_values("step")
                        .iloc[-1]
                        .value
                    )
                    ping_s = (
                        exp_df[exp_df.name == "overlay_ping_signalling_bytes"]
                        .sort_values("step")
                        .iloc[-1]
                        .value
                    )
